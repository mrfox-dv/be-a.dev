# Domain Structure
To register a subdomain, you need to create a new JSON file in the `domains` directory through a pull request. For example, to register `example.be-a.dev`, you would create a file named `example.json` in the `domains` directory. The full path would be `domains/example.json`.

## Filename

**Note**: You can include `.` (dots) in your filename to register a sub-subdomain (e.g., `blog.example.be-a.dev`). However, each segment of your subdomain must meet the following criteria:
...
- `a.be-a.dev.json` (filename contains `.be-a.dev`)
...
```json
{
  "description": "Documentation website for be-a.dev",
  "repo": "https://github.com/is-a-dev/docs",
  "owner": {
    "username": "is-a-dev",
    "email": "admin@be-a.dev"
  },
  "record": {
...
```
Note: Please refer to the [frequently asked questions](https://docs.be-a.dev/faq/) for clarification on what or who we allow NS records for. If you want a example on what we want as the reasonings, you can [checkout this PR](https://github.com/is-a-dev/register/pull/16758).
